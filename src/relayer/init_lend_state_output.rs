...
mod test {
    ...
    #[test]
    fn test_last_state_output_string() {
        // Set test environment variable
        dotenv::from_path(".env").expect("Failed to load .env file");

        // Get the value using our function
        let result = last_state_output_string();

        // Print the result
        println!("Last state output string: {}", result);

        // Verify it matches what we expect
        assert_eq!(result, "");

        // Test default case by removing env var
        let default_result = last_state_output_string();
        println!("Default last state output string: {}", default_result);
