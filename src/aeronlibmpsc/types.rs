...
pub enum StreamId {
    ...
    DirectStreamId = 1000,     //For Single Consumer
}
...
pub fn init_aeron_direct_queue() {
    std::thread::spawn(move || {
        start_aeron_topic_consumer(StreamId::DirectStreamId);
    });
    std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_millis(25));
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct AeronDirectMessage {
    pub topic: StreamId,
    pub msg: String,
}

impl AeronDirectMessage {
    pub fn serialize(&self) -> String {
        let serialized = serde_json::to_string(self).unwrap();
        serialized
    }
    pub fn deserialize(json: &String) -> Self {
        let deserialized: AeronDirectMessage = serde_json::from_str(json).unwrap();
        deserialized
    }
    pub fn extract_msg(self) -> String {
        self.msg
